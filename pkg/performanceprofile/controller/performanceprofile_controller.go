...
func (r *PerformanceProfileReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	currentMCName := profileMCP.Status.Configuration.Name
	if currentMCName == "" { // should never happen
		return reconcile.Result{}, fmt.Errorf("MCP %q for profile %q missing configuration name", profileMCP.Name, instance.Name)
	}
	klog.Infof("checking current MachineConfig %q", currentMCName)

	currentMC, err := r.getMachineConfig(ctx, currentMCName)
	if err != nil {
		return reconcile.Result{}, err
	}
	...
	klog.V(3).Infof("current MachineConfig %q configured for cgroups V1", currentMCName)
